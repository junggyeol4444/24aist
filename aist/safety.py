...
def sanitize_incoming(text: str, author: str = "") -> tuple:
    ...
    a = a.replace(":", " ").strip()
    # 닉네임이 여는 괄호로 시작하면, 코어가 받는 줄이 통째로 괄호로
    # 시작한다 — 페르소나의 무대 규칙은 "괄호로 시작하는 안내는 무대 뒤
    # 신호" 라고 못 박아뒀다. 그러면 두 가지가 한꺼번에 깨진다:
    #   1) 그 시청자의 채팅은 영영 답을 못 받는다(소리 내어 읽지 말라고
    #      돼 있으니까) — 기획 1-2 "다 읽고 다 반응" 위반
    #   2) 닉네임 내용이 지시가 된다. "(조용히 있어" 같은 닉을 쓰면
    #      누구나 방송인에게 무대 뒤 지시를 넣을 수 있다.
    # 빈 닉네임은 이미 막아뒀는데(ANON_AUTHOR) 이 입구가 남아 있었다.
    # 괄호로 읽히는 것만 떼어낸다. 「별하나」 같은 따옴표 닉네임까지
    # 건드리면 멀쩡한 이름만 망가진다.
    a = a.lstrip("([{（［｛ \t").strip()
    if not a:
