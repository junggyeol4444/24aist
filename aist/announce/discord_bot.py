...
class DiscordAnnouncer(Announcer):
    ...
    async def post(self, text: str, *, title: str = "") -> bool:
        ...
        content = text
        allowed = {"parse": []}
        if self.cfg.mention_role_id:
            content = f"<@&{self.cfg.mention_role_id}>\n{text}"
            allowed = {"roles": [str(self.cfg.mention_role_id)]}
        return await asyncio.to_thread(self._post_sync, content, allowed)

    def _post_sync(self, content: str, allowed: dict) -> bool:
        try:
        ...
        try:
            r = requests.post(
                f"{_API}/channels/{self.cfg.channel_id}/messages",
                headers={
                    "Authorization": f"Bot {self.token}",
                    "Content-Type": "application/json",
                },
                json={"content": content[:2000], "allowed_mentions": allowed},
                timeout=15,
            )
