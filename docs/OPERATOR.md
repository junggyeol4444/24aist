...
> 핵심: **사람같음(입 하나·눈치·워밍업·공지 변주·사전 예고)은 기본값**입니다.
> 위 표의 설정들은 그걸 끄기 위한 게 아니라 값을 다듬기 위한 것입니다.
> 시작 시각은 항상 동일(운영자 지시), 채팅 저조 종료·딜레이·flood 만
> 진짜 선택 항목입니다(운영 방침의 문제라서).
...
## 4. 사람같음 = 기본값 (운영자 지시 반영 내역, 2026-07)

운영자 지시: **"사람처럼 방송하는 AI"가 이 제품이다. 사람같음은 선택
옵션이 아니라 기본이다.** 아래 동작은 전부 기본으로 켜져 있으며, 설정은
끄기 위한 것이 아니라 값을 다듬기 위한 것이다:
