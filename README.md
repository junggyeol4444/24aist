...
### 윈도우 — 기본 (더블클릭, 터미널 몰라도 됨)

`windows\` 폴더의 파일을 순서대로 더블클릭하세요 (자세한 건 [`windows/사용법.md`](windows/사용법.md)):

1. **설치.bat** — 처음 한 번. 이거 하나가 다 합니다:
   가상환경 + aist 설치 → 설정 파일 준비 → **방송 코어 준비(웹UI·의존성·`conf.yaml`)** → 점검
2. `config.yaml` / `persona.yaml` / `.env` 를 메모장으로 채우기
3. **점검.bat** — 지금 방송 가능한 상태인지 확인.
   "지금 상태로는 방송이 안 됩니다" 가 뜨면 거기 적힌 `.bat` 부터 실행하세요
4. **코어실행.bat** → **리허설.bat**(플랫폼·키 없이 흐름만) → **테스트방송.bat** → **방송시작.bat**
   (또는 **전체실행.bat** 하나로 코어+자동운영)

**끌 때는 중단.bat**: 방송 창을 X 로 닫으면 윈도우가 프로그램을 바로 죽여서
**OBS 스트림이 켜진 채 남습니다**(시청자에겐 멈춘 화면이 계속 나감).
**중단.bat** 을 다른 창에서 더블클릭하면 마무리 인사 → 스트림 종료 → 기록 저장
까지 하고 끝냅니다. 사고가 났을 때도 이걸 쓰세요. (터미널이면 `aist stop`)

**24시간 무인 운영**: **자동시작등록.bat** — 로그인할 때마다 자동으로 켜지게
윈도우 작업 스케줄러에 등록합니다 (해제는 **자동시작해제.bat**).
컴퓨터가 꺼져 있거나 절전이면 방송도 안 되니, 절전은 꺼두세요.

문제가 생겼을 때:
- 코어 준비만 다시: **코어준비.bat**
- 웹UI(화면)만 다시 받기: **프론트엔드받기.bat**
...
aist rehearse --minutes 3   # 리허설: 플랫폼·키·OBS 없이 흐름만(가짜 채팅)
                            #   코어만 띄워두면 됨. 송출/공지 안 나가고,
                            #   산출물은 data/rehearsal/ 에만 씀(실제 기억 보존)
